...
export default function WriteModal({ symbol, isLoggedIn, onClose, onSuccess }: WriteModalProps) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40" onClick={onClose}>
        ...
        >
          <p className="text-center text-sm text-gray-700 mb-4">글을 작성하려면 로그인이 필요합니다.</p>
          <button
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch('/api/community/posts', {
        ...
        body: JSON.stringify({ ticker_id: symbol, title: title.trim(), content: content.trim(), category }),
      })
      if (!res.ok) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-gray-900">글 작성</h2>
          <button
            onClick={onClose}
